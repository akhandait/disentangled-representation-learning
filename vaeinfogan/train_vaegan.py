...
parser.add_argument('--lrE', type=float, default=3e-4, help='learning rate')
parser.add_argument('--lrD', type=float, default=3e-4, help='learning rate')
parser.add_argument('--lrDis', type=float, default=1e-4, help='learning rate')
...
parser.add_argument('--beta', type=int, default=250, help='the beta (hyper)parameter')
...
parser.add_argument('--outDir', type=str, default='output_vaegan_beta250',
    help='folder to output images and model checkpoints')
# parser.add_argument('--netDisL', type=str, default="temp/Dis_epoch19.pth",
#     help="path to a trained discriminator network for feature-wise loss")
parser.add_argument('--netE', type=str, default='output_vaegan_beta250/checkpoints/E_epoch9.pth',
    help="path to the encoder network (to continue training)")
parser.add_argument('--netD', type=str, default='output_vaegan_beta250/checkpoints/D_epoch9.pth',
    help="path to the decoder network (to continue training)")
parser.add_argument('--netDis', type=str, default='output_vaegan_beta250/checkpoints/Dis_epoch9.pth',
    help="path to the decoder network (to continue training)")
...
try:
    ...
    pass
try:
    os.makedirs(opt.outDir + '/running_samples_continued')
except OSError:
    pass
try:
    os.makedirs(opt.outDir + '/checkpoints_continued')
except OSError:
    pass
...
# DisL = Discriminator(nfDis=opt.nfDis).to(device)
...
# Load weights of a trained Discriminator for feature-wise error.
# DisL.load_state_dict(torch.load(opt.netDisL), strict=False)
# stateDictDisL = torch.load(opt.netDisL)
# for keyDisL, key in zip(DisL.state_dict(), stateDictDisL):
#     DisL.state_dict()[keyDisL] = stateDictDisL[key]
...
for epoch in range(opt.epochs):
    for i, batch in enumerate(dataloader):
        ...
        # E.zero_grad()
        # D.zero_grad()
        # outputReconD = D(R(E(batch.type(Tensor))))
        # featuresReconDisL = DisL.getFeatures(outputReconD)
        # featuresBatchDisL = DisL.getFeatures(batch.type(Tensor))

        # lossDisL = binaryCrossEntropy(featuresReconDisL, featuresBatchDisL.detach())
        # lossDisL = meanSquaredError(featuresReconDisL, featuresBatchDisL.detach()) / batchShape
        ...
        E.zero_grad()
        lossKl = opt.beta * R.klDivergence()
